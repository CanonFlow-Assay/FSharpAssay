...
let analyzeDecl (decl: FSharpImplementationFileDeclaration) (topSups: string list) (sourceText: ISourceText) (compExprRanges: range list) (isTestFile: bool) (hasProperty: bool ref) : Set<Located<Rule>> =
    let rec visitExpr (expr: FSharpExpr) (sups: string list) (inAsync: bool) (inTryFinally: bool) (inLiteral: bool) (inLoop: bool) (assertionsCount: int ref) : Located<Rule> list =
        ...
        | FSharpExprPatterns.Call(obj, func, _, _, args) ->
            ...
            let findings = 
                ...
                if inLoop then
                    if logicalName = "op_Append" || (logicalName = "append" && declaringEntity = "Microsoft.FSharp.Collections.ArrayModule") then
                ...
                if (logicalName = "toList" && declaringEntity = "Microsoft.FSharp.Collections.SeqModule") || (logicalName = "toSeq" && declaringEntity = "Microsoft.FSharp.Collections.ListModule") then
                    if not (isSuppressed currentSups "FSA-P03") then f <- f @ (mkLocated FSAP03 expr.Range |> Option.toList)
                    
                f
            
            let objFindings = match obj with | Some o -> visitExpr o currentSups newInAsync inTryFinally inLiteral inLoop assertionsCount | None -> []
            let argsFindings = args |> List.collect (fun a -> visitExpr a currentSups newInAsync inTryFinally inLiteral inLoop assertionsCount)
        ...
        | FSharpExprPatterns.Let((binding, valExpr, _), body) ->
            ...
            visitExpr valExpr localSups inAsync inTryFinally isLiteralBinding inLoop assertionsCount @ visitExpr body localSups inAsync inTryFinally inLiteral inLoop assertionsCount
        ...
        | FSharpExprPatterns.ValueSet(v, valExpr) ->
            ...
            f @ visitExpr valExpr currentSups inAsync inTryFinally inLiteral inLoop assertionsCount

        | FSharpExprPatterns.Application(func, _, args) ->
            visitExpr func currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ List.collect (fun a -> visitExpr a currentSups inAsync inTryFinally inLiteral inLoop assertionsCount) args
        | FSharpExprPatterns.IfThenElse(cond, ifTrue, ifFalse) ->
            visitExpr cond currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ visitExpr ifTrue currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ visitExpr ifFalse currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        ...
        | FSharpExprPatterns.DecisionTree(cond, targets) ->
            visitExpr cond currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ List.collect (fun (_, e) -> visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount) targets
        | FSharpExprPatterns.DecisionTreeSuccess(_, args) ->
            List.collect (fun a -> visitExpr a currentSups inAsync inTryFinally inLiteral inLoop assertionsCount) args
        | FSharpExprPatterns.Sequential(e1, e2) ->
            ...
            f @ visitExpr e1 currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ visitExpr e2 currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        ...
        | FSharpExprPatterns.LetRec(bindings, body) ->
            ...
            let bindingsFindings = bindings |> List.collect (fun (b, e, _) -> visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount)
            f @ bindingsFindings @ visitExpr body currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        | FSharpExprPatterns.NewObject(ci, _, args) ->
            ...
            f @ List.collect (fun a -> visitExpr a currentSups inAsync inTryFinally inLiteral inLoop assertionsCount) args
        ...
        | FSharpExprPatterns.ObjectExpr(_, baseCall, overrides, interfaceImpls) ->
            visitExpr baseCall currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ 
            List.collect (fun (m: FSharpObjectExprOverride) -> visitExpr m.Body currentSups inAsync inTryFinally inLiteral inLoop assertionsCount) overrides @
            List.collect (fun (_, impls) -> List.collect (fun (m: FSharpObjectExprOverride) -> visitExpr m.Body currentSups inAsync inTryFinally inLiteral inLoop assertionsCount) impls) interfaceImpls
        | FSharpExprPatterns.Quote(e) -> visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        | FSharpExprPatterns.TryFinally(e1, e2, _, _) -> visitExpr e1 currentSups inAsync true inLiteral inLoop assertionsCount @ visitExpr e2 currentSups inAsync true inLiteral inLoop assertionsCount
        | FSharpExprPatterns.TryWith(e1, _, e2, _, e3, _, _) -> 
            ...
            | FSharpExprPatterns.Sequential(_, FSharpExprPatterns.Const(obj, ty)) when ty.HasTypeDefinition && ty.TypeDefinition.LogicalName = "unit" ->
                if not (isSuppressed currentSups "FSA-S03") then f <- f @ (mkLocated FSAS03 expr.Range |> Option.toList)
            | _ -> ()
            f @ visitExpr e1 currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ visitExpr e2 currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ visitExpr e3 currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        | FSharpExprPatterns.UnionCaseTest(e, _, _) -> visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        | FSharpExprPatterns.WhileLoop(cond, body, _) -> visitExpr cond currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ visitExpr body currentSups inAsync inTryFinally inLiteral true assertionsCount
        | FSharpExprPatterns.Coerce(ty, e) -> 
            let mutable f = []
            if ty.HasTypeDefinition && ty.TypeDefinition.LogicalName = "obj" && not (isSuppressed currentSups "FSA-P02") then
                f <- f @ (mkLocated FSAP02 expr.Range |> Option.toList)
            f @ visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        | FSharpExprPatterns.AddressOf(e) -> visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        | FSharpExprPatterns.AddressSet(e1, e2) -> visitExpr e1 currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ visitExpr e2 currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        ...
        | FSharpExprPatterns.UnionCaseSet(e, _, _, _, value) -> visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ visitExpr value currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        | FSharpExprPatterns.UnionCaseTag(e, _) -> visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        | FSharpExprPatterns.FSharpFieldGet(objOpt, _, _) -> match objOpt with Some e -> visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount | None -> []
        | FSharpExprPatterns.FSharpFieldSet(objOpt, _, _, arg) -> (match objOpt with Some e -> visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount | None -> []) @ visitExpr arg currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        | FSharpExprPatterns.ILFieldGet(objOpt, _, _) -> match objOpt with Some e -> visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount | None -> []
        | FSharpExprPatterns.ILFieldSet(objOpt, _, _, arg) -> (match objOpt with Some e -> visitExpr e currentSups inAsync inTryFinally inLiteral inLoop assertionsCount | None -> []) @ visitExpr arg currentSups inAsync inTryFinally inLiteral inLoop assertionsCount
        | FSharpExprPatterns.ILAsm(_, _, args) -> List.collect (fun a -> visitExpr a currentSups inAsync inTryFinally inLiteral inLoop assertionsCount) args
        | FSharpExprPatterns.TraitCall(_, _, _, _, _, args) -> List.collect (fun a -> visitExpr a currentSups inAsync inTryFinally inLiteral inLoop assertionsCount) args
        | FSharpExprPatterns.FastIntegerForLoop(start, limit, body, _, _, _) -> visitExpr start currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ visitExpr limit currentSups inAsync inTryFinally inLiteral inLoop assertionsCount @ visitExpr body currentSups inAsync inTryFinally inLiteral true assertionsCount
        | _ -> []
    ...
    let rec visit (d: FSharpImplementationFileDeclaration) (sups: string list) =
        ...
        | FSharpImplementationFileDeclaration.Entity(e, decls) ->
            ...
            if e.IsValueType then
                if e.FSharpFields.Count > 4 && not (isSuppressed localSups "FSA-P05") then
                    f <- f @ (mkLocated FSAP05 e.DeclarationLocation |> Option.toList)
        ...
        | FSharpImplementationFileDeclaration.InitAction(expr) ->
            let dummyRef = ref 0
            visitExpr expr sups false false false false dummyRef
