...
for definition in candidate policyIdentity toolchain reason counts project source test rule finding exception; do
...
test "$(jq -r '.requiredTests[0].minimumPassed' "$policy")" -eq 83
