...
The executable regression evidence is the 83-test Expecto suite. Both the
ordinary `dotnet test` entry point and direct executable entry point must run all
83 tests, and a deliberately empty selection must fail. Twenty-nine M2 tests exercise
the authority reducer, missing-evidence cases, precedence, invalid evidence, and
cross-root JSON/SARIF determinism.
