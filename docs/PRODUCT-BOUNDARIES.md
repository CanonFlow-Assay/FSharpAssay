...
The executable regression evidence is the 84-test Expecto suite. Both the
ordinary `dotnet test` entry point and direct executable entry point must run all
84 tests, and a deliberately empty selection must fail. Thirty M2 tests exercise
...
files scanned, zero skipped, zero failed evaluations, 546 retained observations,
...
count of 546 are observation counts, not success metrics.
